...
def pts2config(pts : np.ndarray, disc_len: float, closed = False, numbp: int=None, translate_first: bool=True) -> np.ndarray:
    ...
    def find_lambda(p1 : np.ndarray, B : np.ndarray, C : np.ndarray, disc_len : float) -> np.ndarray:
        """
            subsidiary function for pts2config
        """
        w = p1 - B
    ...
    if closed:
        raise ValueError("The option to generate closed configurations is not yet implemented!")
...
def config2triads(config : np.ndarray) -> np.ndarray:
    ...
    for i in range(len(config)-1):
        ...
        if e1[0] != e1[0] or e1[1] != e1[1] or e1[2] != e1[2] or e2[0] != e2[0] or e2[1] != e2[1] or e2[2] != e2[2]:
            print(e1)
            print(e2)
            print(e3)
            sys.exit()
