...
def read_input(filename: str) -> dict:
    ...
    with open(filename,'r') as f:
        ...
        dlines = [line.strip() for line in all_lines if len(line.strip()) > 0 and line.strip()[0] != '#'] 
        for line in dlines:
            if '=' not in line:
                continue
        ...
        for sim in sims:
            ...
            for key in select.keys():
                if key not in sim.keys():
                    ...
                    raise ValueError(f"The argument '{key}' is not contained in the inputfile '{sim['input']}'")
